...
%% Density-Sensitive Regularization
K_median = median(K(:)); % Median kernel value across all kernel entries
C = 1 / K_median; % Adjust C inversely based on median kernel value

low = 0.125; 
high = 10;
lb = low*K_median*ones(2*l,1);
ub = high*K_median*ones(2*l,1);
...
writematrix([mu eta],'mueta_exp2.csv');
